...
from numList import constantsList
...
class Calculator(QWidget):
    ...
    def buttonClicked(self):
        ...
        n = self.display.text()
        functionsList = [calcFunctions.factorial(n), calcFunctions.decToBin(n), calcFunctions.binToDec(n), calcFunctions.decToRoman(n)]
        ...
        for i in range(0, 4):
            if key == constantList[i]:
                self.display.setText(constantsList[i])

        for j in range(0, 4):
            if key == functionList[j]:
                value = functionsList[j]
                self.display.setText(str(value))

        else:
            try:
                resultOper = str(self.display.text())
